##main function
omni.check_v2<-function(N,n,n.iter,burn=1000,thin=4) {#this checks both single and double cancellation
  n/N->dat
  chain<-list()
  #initialize
  inits<-dat
...
############################################################
#glorified wrapper
ConjointChecks_v2<-function(N,n,n.3mat=10,par.options=NULL,seq.seed=NULL,par.seed=NULL) {#N is the number of total tries per cell, n is the number correct
  ...
  proc.fun<-function(dummy,arg.list) {
    ...
    omni.check_v2(nt,nc,n.iter=3000)->out
    #chain.2.ci(out)->out
  ...
  list(omni.check_v2)->lof
  if (is.null(par.options)) {#sequential first
    for (i in 1:n.3mat) {
      list(N,n,lof)->arg.list
    ...
    switch(par.type,
           "snow"={
             ...
             eval(parse(text=final.cmd))
           }
           )
  }
  #for (i in 1:length(out)) posterior[[i]]->out[[i]][[3]]
  list(N=N,n=n,out=out)
}
