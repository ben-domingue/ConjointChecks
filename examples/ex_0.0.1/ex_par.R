...
n.3mat<-6 #not nearly enough, but relatively quick to run.
...
ConjointChecks(tmp$N,tmp$n,n.3mat=n.3mat,seq.seed=123)->out
SummarizeChecks(out)->tab
...
ConjointChecks(tmp$N,tmp$n,n.3mat=n.3mat,par.options=list(c("snow","MPI"),2),seq.seed=3,par.seed=par.seed)->out1a
ConjointChecks(tmp$N,tmp$n,n.3mat=n.3mat,par.options=list(c("snow","SOCK"),2),seq.seed=3,par.seed=par.seed)->out2a
ConjointChecks(tmp$N,tmp$n,n.3mat=n.3mat,par.options=list(c("foreach","doSMP"),2),seq.seed=3,par.seed=par.seed)->out3a
ConjointChecks(tmp$N,tmp$n,n.3mat=n.3mat,par.options=list(c("foreach","doMC"),2),seq.seed=3,par.seed=par.seed)->out4a
...
######################
########################
#if the above works, let's look harder on janus
library(ConjointChecks)
library(Rmpi)
dat<-list()
n.items<-20
...
##################3
##################3
setwd("/home/bd/Dropbox/conjoint/package/examples")
load("pkg.Rdata")
res<-list()
library(ConjointChecks)
par(mfrow=c(2,2))
for (i in 1:3) {
  load(paste("dat",i,".Rdata",sep=""))
  SummarizeChecks(tmp)->res[[i]]
  PlotChecks(res[[i]])
}
